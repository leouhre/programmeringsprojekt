...
void powerup_render(powerup_t *powerups, spaceship_t *sh, uint8_t *nukeCount) { //draws the powerup in putty depending on collision
    ...
    for(i = 0; i < 5; i++) {
        if(powerups[i].alive == 1) { // only draw existing powerups
            if(!playerPowerupCollision(powerups[i], sh, nukeCount)) {
...
uint8_t playerPowerupCollision(powerup_t powerup, spaceship_t *sh, uint8_t *nukeCount) {
